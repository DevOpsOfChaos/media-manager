# workflow history-summary-by-command

Ziel: neuer additiver Workflow-CLI-Command auf Basis der bereits vorhandenen History-Core-Helfer, **ohne** die bestehenden JSON-Verträge von `history` oder `last` zu verändern.
...
Die benötigten Core-Helfer sind auf `main` bereits vorhanden.

---

## 1) Importblock erweitern

Im Import aus `.core.state` `summarize_history_entries_by_command` ergänzen.
...
Direkt **nach** `history-latest-by-command` einen neuen Parser hinzufügen.
...
## 5) Hilfe-Block erweitern

Im großen Hilfe-Text in `main(...)` eine Beispielzeile ergänzen:

```python
            "  media-manager workflow history-summary-by-command --path <RUNS_DIR> --summary-only\n"
```

Sinnvoll direkt **nach**:
...
## 6) Dispatch in `main(...)` ergänzen

Direkt **nach** dem bestehenden Block für `history-latest-by-command`:
...
```powershell
pytest -q tests/test_cli_workflow_history_summary_by_command_v1.py
pytest -q tests/test_cli_workflow_history_v2.py tests/test_cli_workflow_history_latest_by_command_v1.py tests/test_core_workflow_history_v2.py
```

Mittlerer Lauf:

```powershell
pytest -q tests/test_cli_workflow_history_v2.py tests/test_cli_workflow_history_latest_by_command_v1.py tests/test_cli_workflow_history_summary_by_command_v1.py tests/test_core_workflow_history_v2.py
```
...
## Abschlussblock für den User-Repo-Stand

```powershell
git add src/media_manager/cli_workflow.py tests/test_cli_workflow_history_summary_by_command_v1.py
git commit -m "feat: add workflow history summary-by-command CLI command"
git push
```
