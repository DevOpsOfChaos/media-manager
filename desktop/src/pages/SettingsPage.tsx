...
import {
  FolderSearch, X, Check, RotateCcw,
  Settings, FolderOpen, ScanFace, Cpu, Download, Upload,
} from "lucide-react"
...
const CATEGORIES = [
  ...
  { id: "faces", icon: ScanFace },
  { id: "system", icon: Cpu },
] as const

type CategoryId = (typeof CATEGORIES)[number]["id"]

const CATEGORY_LABELS: Record<CategoryId, [string, string]> = {
  general: ["General", "Allgemein"],
  library: ["Library", "Bibliothek"],
  faces: ["Face Recognition", "Gesichtserkennung"],
...
export default function SettingsPage() {
  ...
  const validateField = (key: string, value: any) => {
    ...
    if (key === "face_recognition_tolerance") {
      ...
      if (isNaN(num) || num < 0.1 || num > 1.0) {
        newErrors[key] = t("Must be between 0.1 and 1.0", "Muss zwischen 0.1 und 1.0 liegen")
      }
    }
    if (key === "default_source_dir" && value && !value.trim()) {
  ...
  const exportSettings = () => {
    ...
    a.href = url
    a.download = "media-manager-settings.json"
    a.click()
    ...
    input.onchange = (e) => {
      ...
      reader.onload = () => {
        try {
          ...
          setSettings({ ...defaultSettings, ...settings, ...imported })
          setLastModified(new Date())
          toast("success", t("Settings imported", "Einstellungen importiert"))
        } catch {
          toast("error", t("Invalid settings file", "Ungültige Einstellungsdatei"))
        }
      }
        ...
        <div className="flex-1 max-w-5xl space-y-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Button variant="outline" size="sm" onClick={exportSettings}>
              <Download className="h-3 w-3 mr-1" /> {t("Export", "Exportieren")}
            </Button>
            <Button variant="outline" size="sm" onClick={importSettings}>
              <Upload className="h-3 w-3 mr-1" /> {t("Import", "Importieren")}
            </Button>
            {dirty && (
            ...
            {lastModified && (
              <div className="text-[10px] text-muted-foreground">
                {t("Last saved", "Zuletzt gespeichert")}: {lastModified.toLocaleString()}
              </div>
            )}
          </div>
          ...
          <div className="flex gap-1 border-b pb-2">
            {CATEGORIES.map((cat) => {
              const Icon = cat.icon
              const [en, de] = CATEGORY_LABELS[cat.id]
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      setDefaultSourceDir(e.target.value)
                      validateField("default_source_dir", e.target.value)
                      localStorage.setItem("default_source_dir", e.target.value)
                    }}
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      validateField("face_recognition_tolerance", val)
