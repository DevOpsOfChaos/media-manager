    ...
    <div className="fixed inset-0 z-50 bg-black/80 flex flex-col">
      <div className="flex items-center justify-between px-4 py-2 bg-black/60 text-white">
        <div className="flex items-center gap-2 text-xs">
          ...
          <span>{files[leftIndex]?.name}</span>
          <ArrowLeftRight className="h-3 w-3 opacity-40" />
          <span>{files[rightIndex]?.name}</span>
        </div>
      ...
      <div className="flex-1 flex">
        <div className="flex-1 flex items-center justify-center bg-black/40 p-4">
          <img src={convertFileSrc(files[leftIndex]?.path)} alt={files[leftIndex]?.name}
            className="max-w-full max-h-full object-contain" />
        </div>
        <div className="w-px bg-white/20" />
        <div className="flex-1 flex items-center justify-center bg-black/40 p-4">
          <img src={convertFileSrc(files[rightIndex]?.path)} alt={files[rightIndex]?.name}
            className="max-w-full max-h-full object-contain" />
        </div>
      </div>
