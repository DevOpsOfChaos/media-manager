...
from media_manager.core.gui_qt_runtime_smoke_event_map import build_qt_runtime_smoke_event_map
from media_manager.core.gui_qt_runtime_smoke_interactions import build_qt_runtime_smoke_interaction_plan


def test_event_map_creates_safe_future_signal_mappings() -> None:
    interaction = build_qt_runtime_smoke_interaction_plan(sample_visible_surface())
    event_map = build_qt_runtime_smoke_event_map(interaction)
    assert event_map["summary"]["event_count"] == 2
    assert event_map["summary"]["confirmation_event_count"] == 1
    assert event_map["summary"]["immediate_execution_count"] == 0
    assert event_map["events"][0]["qt_signal"] == "clicked"
    assert event_map["events"][1]["handler_name"] == "handle_start_manual_qt_smoke"
