...
from media_manager.core.gui_qt_runtime_smoke_detail_panel import build_qt_runtime_smoke_detail_panel


def test_runtime_smoke_detail_panel_keeps_manual_confirmation_and_privacy() -> None:
    detail = build_qt_runtime_smoke_detail_panel(sample_workbench())

    assert detail["kind"] == "qt_runtime_smoke_detail_panel"
    assert detail["decision"]["requires_user_confirmation"] is True
    assert detail["decision"]["executes_immediately"] is False
    assert detail["privacy"]["local_only"] is True
    assert detail["privacy"]["network_required"] is False
    assert detail["trend"]["direction"] == "improved"
