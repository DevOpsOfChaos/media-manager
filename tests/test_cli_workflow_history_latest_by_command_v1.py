...
def test_history_latest_by_command_json_returns_one_latest_entry_per_command(tmp_path: Path, capsys) -> None:
    logs = tmp_path / "logs"
    logs.mkdir()
    _write_run_log(logs / "rename-older.json", command_name="rename", created_at_utc="2026-04-18T09:00:00+00:00")
    _write_run_log(logs / "rename-newer.json", command_name="rename", created_at_utc="2026-04-18T10:00:00+00:00")
    _write_run_log(logs / "organize.json", command_name="organize", created_at_utc="2026-04-18T11:00:00+00:00")

    exit_code = main(["history-latest-by-command", "--path", str(logs), "--json"])
    ...
    assert [item["command_name"] for item in payload["entries"]] == ["organize", "rename"]
    assert payload["entries"][1]["path"].endswith("rename-newer.json")


def test_history_latest_by_command_forwards_new_history_filters(monkeypatch, capsys, tmp_path: Path) -> None:
    captured_kwargs: dict[str, object] = {}

    def fake_find(path: Path, **kwargs):
    ...
    monkeypatch.setattr("media_manager.cli_workflow.find_latest_history_entries_by_command", fake_find)
    ...
    assert payload["entries"][0]["command_name"] == "trip"


def test_history_latest_by_command_summary_only_text_output(tmp_path: Path, capsys) -> None:
    logs = tmp_path / "logs"
    logs.mkdir()
    ...
    exit_code = main(["history-latest-by-command", "--path", str(logs), "--summary-only"])
    captured = capsys.readouterr()

    assert exit_code == 0
    assert "Latest workflow history entry per command" in captured.out
...
def test_history_latest_by_command_fail_on_empty_returns_one(tmp_path: Path, capsys) -> None:
    logs = tmp_path / "logs"
    logs.mkdir()

    exit_code = main(["history-latest-by-command", "--path", str(logs), "--fail-on-empty"])
    captured = capsys.readouterr()
