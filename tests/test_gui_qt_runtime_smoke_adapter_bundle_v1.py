...
def test_adapter_bundle_composes_visible_surface_runtime_contracts() -> None:
    bundle = build_qt_runtime_smoke_adapter_bundle(sample_visible_surface())
    assert bundle["kind"] == "qt_runtime_smoke_adapter_bundle"
    assert bundle["ready_for_qt_runtime"] is True
    assert bundle["summary"]["render_step_count"] > 0
...
def test_adapter_bundle_not_ready_when_component_is_unsupported() -> None:
    bundle = build_qt_runtime_smoke_adapter_bundle(sample_visible_surface(unsupported=True))
    assert bundle["ready_for_qt_runtime"] is False
    assert bundle["summary"]["problem_count"] == 1
