...
def test_widget_tree_builds_root_sections_and_items() -> None:
    tree = build_qt_runtime_smoke_widget_tree(sample_visible_surface())
    assert tree["summary"]["section_count"] == 4
    assert tree["summary"]["item_node_count"] == 4
    assert tree["summary"]["unsupported_component_count"] == 0
    assert tree["summary"]["executable_node_count"] == 0
    assert tree["root"]["sensitive"] is True


def test_widget_tree_reports_unsupported_components() -> None:
    tree = build_qt_runtime_smoke_widget_tree(sample_visible_surface(unsupported=True))
    assert tree["summary"]["unsupported_component_count"] == 1
    assert "UnsupportedActions" in tree["summary"]["unsupported_components"]
