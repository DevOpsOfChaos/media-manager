...
from media_manager.core.gui_qt_runtime_smoke_adapter_validation import validate_qt_runtime_smoke_adapter_bundle


def test_adapter_validation_accepts_supported_local_bundle() -> None:
    bundle = build_qt_runtime_smoke_adapter_bundle(sample_visible_surface())
    validation = validate_qt_runtime_smoke_adapter_bundle(bundle)
    assert validation["valid"] is True
    assert validation["problem_count"] == 0
    assert validation["summary"]["render_step_count"] > 0
    assert validation["summary"]["binding_count"] == 2
    assert validation["summary"]["local_only"] is True


def test_adapter_validation_rejects_unsupported_component() -> None:
    bundle = build_qt_runtime_smoke_adapter_bundle(sample_visible_surface(unsupported=True))
    validation = bundle["validation"]
    assert validation["valid"] is False
    assert validation["problem_count"] == 1
    assert validation["problems"][0]["code"] == "unsupported_components"
