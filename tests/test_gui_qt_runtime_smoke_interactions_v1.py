...
def test_interaction_plan_keeps_actions_deferred_or_confirmed() -> None:
    plan = build_qt_runtime_smoke_interaction_plan(sample_visible_surface())
    assert plan["summary"]["binding_count"] == 2
    assert plan["summary"]["enabled_binding_count"] == 2
    assert plan["summary"]["confirmation_binding_count"] == 1
    assert plan["summary"]["immediate_execution_count"] == 0
    assert plan["bindings"][1]["execute_policy"] == "manual_confirm"
