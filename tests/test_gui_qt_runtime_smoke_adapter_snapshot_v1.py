...
from media_manager.core.gui_qt_runtime_smoke_adapter_snapshot import build_qt_runtime_smoke_adapter_snapshot


def test_adapter_snapshot_is_stable_and_metadata_only() -> None:
    bundle = build_qt_runtime_smoke_adapter_bundle(sample_visible_surface())
    snapshot_a = build_qt_runtime_smoke_adapter_snapshot(bundle)
    snapshot_b = build_qt_runtime_smoke_adapter_snapshot(bundle)
    assert snapshot_a["payload_hash"] == snapshot_b["payload_hash"]
    assert len(snapshot_a["payload_hash"]) == 64
    assert snapshot_a["summary"]["valid"] is True
