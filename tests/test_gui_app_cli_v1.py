...
def test_gui_app_json_is_headless(tmp_path: Path, capsys) -> None:
    assert main(["--json", "--active-page", "dashboard", "--profile-dir", str(tmp_path / "profiles")]) == 0
    payload = json.loads(capsys.readouterr().out)

    assert payload["application"]["id"] == "media-manager"
    assert payload["active_page_id"] == "dashboard"
    assert payload["capabilities"]["tkinter_shell"] is True


def test_gui_app_summary_is_headless(capsys) -> None:
    assert main(["--summary", "--active-page", "profiles"]) == 0
