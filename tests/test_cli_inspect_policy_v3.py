...
def test_cli_inspect_json_includes_decision_policy_and_conflict_summary(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    def resolve_stub(path: Path, inspection=None, exiftool_path=None) -> DateResolution:
        ...
        return DateResolution(
            path=path,
            resolved_datetime=dt,
            resolved_value="2024-08-10 11:12:14",
    ...
    assert code == 0
    assert payload["summary"]["decision_policy_summary"] == {
        "highest_priority_parseable_metadata": 1,
        "mtime_fallback": 1,
    }
    assert payload["summary"]["metadata_conflict_count"] == 1
    assert payload["files"][0]["resolution"]["decision_policy"] == "highest_priority_parseable_metadata"
    assert payload["files"][0]["resolution"]["metadata_conflict"] is True


def test_cli_inspect_text_output_prints_decision_policy_summary(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    assert code == 0
    assert "Decision policies: highest_priority_parseable_metadata=1" in output
    assert "Metadata conflict:  False" in output
