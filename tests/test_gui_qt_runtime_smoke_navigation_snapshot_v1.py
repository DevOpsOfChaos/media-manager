...
from media_manager.core.gui_qt_runtime_smoke_navigation_snapshot import build_qt_runtime_smoke_navigation_snapshot
from media_manager.core.gui_qt_runtime_smoke_page_handoff import build_qt_runtime_smoke_page_handoff


def test_navigation_snapshot_is_stable_metadata_only() -> None:
    handoff = build_qt_runtime_smoke_page_handoff(sample_adapter_bundle(), existing_pages=sample_existing_pages())
    snapshot_a = build_qt_runtime_smoke_navigation_snapshot(handoff)
    snapshot_b = build_qt_runtime_smoke_navigation_snapshot(handoff)

    assert snapshot_a["payload_hash"] == snapshot_b["payload_hash"]
    assert len(snapshot_a["payload_hash"]) == 64
    assert snapshot_a["summary"]["ready_for_shell_registration"] is True
