...
$MainQmlPath = Join-Path $RepoRoot 'src\media_manager\qml\Main.qml'

if (-not (Test-Path $MainQmlPath)) {
    throw "Main.qml not found at $MainQmlPath. Run this script from the repo root."
}

$content = Get-Content -Path $MainQmlPath -Raw -Encoding UTF8

$pattern = '(?s)Rectangle \{\r?\n\s+Layout\.fillWidth: true\r?\n\s+implicitHeight: dryRunColumn\.implicitHeight \+ 28\r?\n\s+radius: 18\r?\n\s+color: "#0F1A2C"\r?\n\s+border\.color: "#22324A"\r?\n\s+.*?\r?\n\s+\}\r?\n(?=\s*Rectangle \{\r?\n\s+Layout\.fillWidth: true\r?\n\s+implicitHeight: plannedColumn\.implicitHeight \+ 28)'
...
if (-not [regex]::IsMatch($content, $pattern)) {
    throw "Expected dry-run block not found. Stop here and do not overwrite the file blindly."
}

$newContent = [regex]::Replace($content, $pattern, $replacement, 1)

$backupPath = "$MainQmlPath.bak_before_summary_fix"
Set-Content -Path $backupPath -Value $content -Encoding UTF8
Set-Content -Path $MainQmlPath -Value $newContent -Encoding UTF8

Write-Host "Updated: $MainQmlPath"
