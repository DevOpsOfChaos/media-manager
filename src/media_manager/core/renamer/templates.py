...
def render_rename_filename(
    ...
    *,
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        if token_name == "minute":
            return dt.strftime("%M")
        if token_name == "second":
            return dt.strftime("%S")
        if token_name == "year_month":
            return dt.strftime("%Y-%m")
        if token_name == "year_month_day":
            return dt.strftime("%Y-%m-%d")
        if token_name == "stem":
            return sanitize_filename(file_path.stem)
        if token_name == "suffix":
            return file_path.suffix
        if token_name == "source_name":
            return sanitize_filename(source_root.name if source_root is not None else "")
        if token_name == "index":
            return f"{index:04d}"
    ...
    if includes_suffix:
        ...
        base_name = cleaned + file_path.suffix
