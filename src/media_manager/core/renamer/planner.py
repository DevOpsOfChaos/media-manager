...
def build_rename_dry_run(options: RenamePlannerOptions) -> RenameDryRun:
    ...
    for index, scanned_file in enumerate(scan_summary.files, start=1):
        try:
            ...
            dry_run.entries.append(RenamePlanEntry(scanned_file=scanned_file, resolution=None, status="error", reason=str(exc), rendered_name=None, target_path=None))
            continue

        source_key = _normalized_path_key(scanned_file.path)
        target_key = _normalized_path_key(target_path)
        if source_key == target_key:
            dry_run.entries.append(RenamePlanEntry(scanned_file=scanned_file, resolution=resolution, status="skipped", reason="source file already matches the planned rename target", rendered_name=rendered_name, target_path=target_path))
            continue
        if target_path.exists():
            dry_run.entries.append(RenamePlanEntry(scanned_file=scanned_file, resolution=resolution, status="conflict", reason="target file name already exists in the source directory", rendered_name=rendered_name, target_path=target_path))
            continue
        dry_run.entries.append(RenamePlanEntry(scanned_file=scanned_file, resolution=resolution, status="planned", reason="ready for rename execution", rendered_name=rendered_name, target_path=target_path))
