...
def _evaluate_group_plan_state(group_target_paths: dict[Path, Path]) -> tuple[str, str]:
...
def _augment_files_with_associated_sidecars(files: list[ScannedFile]) -> list[ScannedFile]:
...
def build_organize_dry_run(options: OrganizePlannerOptions) -> OrganizeDryRun:
    ...
    if options.include_associated_files:
        scanned_files = _augment_files_with_associated_sidecars(scanned_files)
    ...
    if options.include_associated_files:
        ...
        for group in groups:
            ...
            try:
                ...
                status, reason = _evaluate_group_plan_state(group_target_paths)
