    ...
    sections = [
        {
            ...
            "kind": "status_strip",
            "items": list(_list(status_strip.get("badges"))),
        },
        {
        ...
        },
        {
            ...
            "kind": "trend",
            "items": list(_list(trend.get("points"))),
            "summary": dict(_mapping(trend.get("summary"))),
        },
    ]
...
def summarize_qt_runtime_smoke_workbench(workbench: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "Qt runtime smoke workbench",
            f"  Active page: {workbench.get('active_page_id')}",
            f"  Status: {workbench.get('status')}",
            f"  Sections: {summary.get('section_count', 0)}",
            f"  Cards: {summary.get('card_count', 0)}",
            f"  Ready: {summary.get('ready_for_runtime_review')}",
        ]
    )


__all__ = [
    "QT_RUNTIME_SMOKE_WORKBENCH_SCHEMA_VERSION",
    "build_qt_runtime_smoke_workbench",
    "summarize_qt_runtime_smoke_workbench",
]
